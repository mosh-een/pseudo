...
<title>PseudoCoding by MrC</title>
<script>
  // Apply saved theme immediately to avoid a flash of the wrong theme
  (function () {
    try {
      if (localStorage.getItem('pcp_theme') === 'light') {
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .theme-toggle-btn { background: var(--bg3); border: 1px solid var(--border); color: var(--text); width: 26px; height: 26px; border-radius: 50%; cursor: pointer; font-size: 0.85rem; display: flex; align-items: center; justify-content: center; transition: all 0.15s; }
  .theme-toggle-btn:hover { background: var(--border); transform: scale(1.08); }
  ...
  .builtin-item { padding: 5px 8px; border-radius: 4px; cursor: pointer; font-size: 0.75rem; transition: all 0.15s; }
  .builtin-item:hover { background: var(--border); }
  .builtin-item .bi-name { color: var(--green); font-family: var(--font-mono); font-weight: 600; }
  .builtin-item .bi-args { color: var(--text2); font-family: var(--font-mono); }
  .builtin-item .bi-desc { color: var(--text2); font-size: 0.68rem; margin-top: 1px; }
  /* Floating built-in functions button + popover */
  .builtin-fab { position: fixed; bottom: 16px; left: 16px; z-index: 300; }
  .builtin-fab-btn {
    background: var(--bg3); border: 1px solid var(--border); color: var(--text2);
    padding: 6px 11px; border-radius: 20px; cursor: grab; font-size: 0.75rem;
    display: flex; align-items: center; gap: 5px; white-space: nowrap;
    transition: background 0.15s, color 0.15s; user-select: none; box-shadow: 0 2px 8px rgba(0,0,0,0.4);
  }
  .builtin-fab-btn:hover { background: var(--border); color: var(--text); }
  .builtin-fab-btn.dragging { cursor: grabbing; opacity: 0.85; }
  .builtin-popover {
    position: absolute; bottom: calc(100% + 8px); left: 0;
    width: 230px; max-height: 420px;
    background: var(--bg2); border: 1px solid var(--border); border-radius: 8px;
    box-shadow: 0 8px 24px rgba(0,0,0,0.5); overflow-y: auto;
    display: none; flex-direction: column;
  }
  .builtin-popover.open { display: flex; }
  .builtin-popover.flip-down { bottom: auto; top: calc(100% + 8px); }
  .builtin-pop-header { padding: 8px 12px; font-size: 0.68rem; text-transform: uppercase;
    letter-spacing: 0.1em; color: var(--text2); border-bottom: 1px solid var(--border);
    position: sticky; top: 0; background: var(--bg2); flex-shrink: 0; }
  .builtin-pop-body { padding: 4px; overflow-y: auto; }
  ...
  .code-editor { flex: 1; background: var(--code-bg); color: var(--text); font-family: var(--font-mono); font-size: 14px; line-height: 1.6; border: none; outline: none; resize: none; padding: 12px 12px 12px 52px; tab-size: 4; spellcheck: false; }
  .line-numbers { position: absolute; left: 0; top: 0; bottom: 0; width: 44px; background: var(--gutter-bg); padding: 12px 0; font-family: var(--font-mono); font-size: 14px; line-height: 1.6; color: var(--text2); text-align: right; user-select: none; overflow: hidden; pointer-events: auto; cursor: pointer; z-index: 1; }
  ...
  .output-line { margin-bottom: 2px; white-space: pre-wrap; word-break: break-word; tab-size: 4; }
  ...
  .fn { color: var(--tok-fn); }
  .typ { color: var(--tok-typ); }

  /* ── Debugger toolbar ── */
  .debug-bar { display: none; background: var(--code-bg); border-bottom: 1px solid var(--border); padding: 5px 12px; gap: 6px; align-items: center; flex-shrink: 0; }
...
<header>
  <div class="logo">Pseudo<span>Coding</span> <span style="font-size:0.65rem;color:var(--accent2)">MrC</span></div>
...
function toggleTheme() {
  ...
  localStorage.setItem('pcp_theme', nowLight ? 'light' : 'dark');
  document.getElementById('themeToggle').textContent = nowLight ? '☀️' : '🌙';
  // Re-render any currently displayed converted code so token colors refresh correctly
  if (currentTab === 'python' || currentTab === 'vb') switchTab(currentTab);
}
...
// ─── Built-in function reference (per CIE syllabus) ───────────────────────────
// 2210 = IGCSE/O-Level pseudocode guide (small core set)
// 9618 = A-Level pseudocode guide (adds LEFT/RIGHT/MID, RANDOMBETWEEN, ABS, etc.)
const builtinFunctions = {
  '2210': [
    { name: 'LENGTH',    args: '(String)',                 desc: 'Number of characters in a string',      insert: 'LENGTH(myString)' },
    { name: 'UCASE',     args: '(Char)',                    desc: 'Converts a character to upper case',     insert: 'UCASE(myChar)' },
    { name: 'LCASE',     args: '(Char)',                    desc: 'Converts a character to lower case',     insert: 'LCASE(myChar)' },
    { name: 'SUBSTRING', args: '(String, start, length)',   desc: 'Extracts part of a string (1-indexed)',  insert: 'SUBSTRING(myString, 1, 3)' },
    { name: 'ROUND',     args: '(Real, places)',             desc: 'Rounds a real number to N places',       insert: 'ROUND(myReal, 2)' },
    { name: 'RANDOM',    args: '()',                         desc: 'Random REAL between 0 and 1',            insert: 'RANDOM()' },
    { name: 'INT',       args: '(Real)',                     desc: 'Integer part of a real number',          insert: 'INT(myReal)' },
    { name: 'ASC',       args: '(Char)',                     desc: 'ASCII code of a character',              insert: 'ASC(myChar)' },
    { name: 'CHR',       args: '(Integer)',                  desc: 'Character for an ASCII code',            insert: 'CHR(65)' },
  ],
  '9618': [
    { name: 'LENGTH',       args: '(String)',                 desc: 'Number of characters in a string',         insert: 'LENGTH(myString)' },
    { name: 'UCASE',        args: '(Char)',                   desc: 'Converts a character to upper case',       insert: 'UCASE(myChar)' },
    { name: 'LCASE',        args: '(Char)',                   desc: 'Converts a character to lower case',       insert: 'LCASE(myChar)' },
    { name: 'LEFT',         args: '(String, n)',               desc: 'Leftmost n characters of a string',        insert: 'LEFT(myString, 3)' },
    { name: 'RIGHT',        args: '(String, n)',               desc: 'Rightmost n characters of a string',       insert: 'RIGHT(myString, 3)' },
    { name: 'MID',          args: '(String, start, length)',   desc: 'Substring from start, of given length',   insert: 'MID(myString, 2, 3)' },
    { name: 'ROUND',        args: '(Real, places)',            desc: 'Rounds a real number to N places',         insert: 'ROUND(myReal, 2)' },
    { name: 'INT',          args: '(Real)',                    desc: 'Integer part of a real number',            insert: 'INT(myReal)' },
    { name: 'ABS',          args: '(Number)',                  desc: 'Absolute value of a number',               insert: 'ABS(myNumber)' },
    { name: 'RANDOM',       args: '()',                        desc: 'Random REAL between 0 and 1',              insert: 'RANDOM()' },
    { name: 'RANDOMBETWEEN',args: '(low, high)',               desc: 'Random INTEGER between low and high',     insert: 'RANDOMBETWEEN(1, 6)' },
    { name: 'ASC',          args: '(Char)',                    desc: 'ASCII code of a character',                insert: 'ASC(myChar)' },
    { name: 'CHR',          args: '(Integer)',                 desc: 'Character for an ASCII code',              insert: 'CHR(65)' },
    { name: 'STR_TO_NUM',   args: '(String)',                  desc: 'Converts a string to a number',            insert: 'STR_TO_NUM(myString)' },
    { name: 'NUM_TO_STR',   args: '(Number)',                  desc: 'Converts a number to a string',            insert: 'NUM_TO_STR(myNumber)' },
    { name: 'EOF',          args: '(filename)',                desc: 'TRUE if at end of an open file',           insert: 'EOF("data.txt")' },
  ],
};
...
function renderBuiltins() {
  ...
  if (!list) return;
  list.innerHTML = '';
  const fns = builtinFunctions[syllabus] || builtinFunctions['2210'];
  for (const fn of fns) {
    const el = document.createElement('div');
    el.className = 'builtin-item';
    el.title = `${fn.name}${fn.args} — ${fn.desc}`;
    el.innerHTML = `<div><span class="bi-name">${escHtml(fn.name)}</span><span class="bi-args">${escHtml(fn.args)}</span></div><div class="bi-desc">${escHtml(fn.desc)}</div>`;
    el.onclick = () => { insertSnippetAtCursor(fn.insert); closeBuiltinPopover(); };
    list.appendChild(el);
  }
  const header = document.getElementById('builtinPopHeader');
  if (header) header.textContent = `🔧 Built-ins (${syllabus})`;
  const label = document.getElementById('builtinFabLabel');
  if (label) label.textContent = `Built-ins (${syllabus})`;
}

function toggleBuiltinPopover() {
  const pop = document.getElementById('builtinPopover');
  const fab = document.getElementById('builtinFab');
  if (!pop || !fab) return;
  const fabRect = fab.getBoundingClientRect();
  // Flip below the button if there's not enough room above
  if (fabRect.top < 440) {
    pop.classList.add('flip-down');
  } else {
...
document.addEventListener('click', function(e) {
  const fab = document.getElementById('builtinFab');
  if (fab && !fab.contains(e.target)) closeBuiltinPopover();
});
...
(function() {
  ...
  function initFabDrag() {
    const fab = document.getElementById('builtinFab');
    const btn = document.getElementById('builtinFabBtn');
    if (!fab || !btn) return;

    // Resolve initial bottom/left into top/left so we can move freely
    ...
    btn.addEventListener('mousedown', function(e) {
      ...
      function onMove(e) {
        ...
        if (dragging) {
          ...
          fab.style.left = Math.max(0, Math.min(maxL, startLeft + dx)) + 'px';
          fab.style.top  = Math.max(0, Math.min(maxT, startTop  + dy)) + 'px';
        }
      }
      ...
      function onUp() {
        ...
        // Save position
        localStorage.setItem('pcp_fab_pos', JSON.stringify({ left: fab.style.left, top: fab.style.top }));
      }
    ...
    // Suppress click after a drag so the popover doesn't toggle
    ...
    try {
      const saved = JSON.parse(localStorage.getItem('pcp_fab_pos') || 'null');
...
async function execLinesDebug(lines, env, startIdx, originalLines) {
  ...
  while (i < lines.length) {
    ...
    // Skip over FUNCTION/PROCEDURE definition blocks entirely — they were
    // already collected in the first pass; executing them again here would
    // wrongly terminate the whole program when RETURN is reached.
    if (/^FUNCTION\b/i.test(src) || /^PROCEDURE\b/i.test(src)) {
      let depth = 1;
      i++;
      while (i < lines.length && depth > 0) {
        if (/^FUNCTION\b/i.test(lines[i]) || /^PROCEDURE\b/i.test(lines[i])) depth++;
        if (/^ENDFUNCTION\b/i.test(lines[i])) { depth--; i++; if (depth === 0) break; continue; }
        if (/^ENDPROCEDURE\b/i.test(lines[i])) { depth--; i++; if (depth === 0) break; continue; }
        i++;
      }
      continue;
    }
    ...
    // ── IF — handled here (instead of delegating to execLine) so the body
    // is run through execLinesDebug and pauses per-line like everything else.
    if (/^IF\s.+THEN$/i.test(src)) {
      const cond = src.replace(/^IF\s+/i, '').replace(/\s+THEN$/i, '');
      const ifLines = [], elseLines = [];
      let j = i + 1, depth = 1, inElse = false;
      ...
      const res = evalCond(cond, env)
        ? await execLinesDebug(ifLines, env, 0, originalLines)
        : await execLinesDebug(elseLines, env, 0, originalLines);
      if (debugStopped) return;
    ...
    // ── FOR ──────────────────────────────────────────────────────────────
    const forMatch = src.match(/^FOR\s+(\w+)\s*(?:←|<-|<--|←-)\s*(.+?)\s+TO\s+(.+?)(?:\s+STEP\s+(.+))?$/i);
    if (forMatch) {
      const varName = forMatch[1].toUpperCase();
      const start = evalExpr(forMatch[2], env);
      ...
      for (let v = start; step > 0 ? v <= end : v >= end; v += step) {
        ...
        env.vars[varName] = v;
        const res = await execLinesDebug(body, env, 0, originalLines);
        if (debugStopped) return;
        if (res && res.type === 'return') return res;
      }
      i = j + 1; continue;
    ...
    // ── WHILE ────────────────────────────────────────────────────────────
    if (/^WHILE\s.+\s+DO$/i.test(src)) {
      const cond = src.replace(/^WHILE\s+/i, '').replace(/\s+DO$/i, '');
      const body = [];
      let j = i + 1, depth = 1;
      while (j < lines.length && depth > 0) {
        const l = lines[j];
    ...
    // ── REPEAT...UNTIL ───────────────────────────────────────────────────
    if (/^REPEAT$/i.test(src)) {
      const body = [];
      let j = i + 1;
      while (j < lines.length && !/^UNTIL\s/i.test(lines[j])) { body.push(lines[j]); j++; }
      const untilCond = lines[j]?.replace(/^UNTIL\s+/i, '');
      let guard = 0;
      do {
        if (debugStopped) return;
        if (++guard > 100000) throw new Error('Loop limit exceeded');
    ...
    // ── CASE OF ──────────────────────────────────────────────────────────
    if (/^CASE\s+OF\s+/i.test(src)) {
      const varExpr = src.replace(/^CASE\s+OF\s+/i, '').trim();
      ...
      if (chosen) {
        const res = await execLinesDebug([chosen], env, 0, originalLines);
        if (debugStopped) return;
        if (res && res.type === 'return') return res;
      }
      i = j + 1; continue;
...
function switchTab(tab) {
  ...
  if (tab !== 'pseudocode') {
    const code = document.getElementById('codeEditor').value;
    if (tab === 'python') showConverted(toPython(code), 'Python', 'python');
    if (tab === 'vb') showConverted(toVB(code), 'VB.NET (Console)', 'vb');
  }
}

function showConverted(code, lang, langKey) {
  clearOutput();
  addOutput(`// Converted to ${lang} (approximate)`, 'sys');
  addOutput('// ─────────────────────────────', 'sys');
  code.split('\n').forEach(l => addCodeOutput(l, 'out', langKey));
}

// ─── Syntax highlighting for converted-code output ─────────────────────────
const VB_TOKEN_RE = new RegExp(
  "(?<comment>'.*)" +
  '|(?<string>"(?:[^"\\\\]|\\\\.)*")' +
  '|(?<number>\\b\\d+(?:\\.\\d+)?\\b)' +
  '|(?<type>\\b(?:Integer|Double|String|Boolean|Char|Object)\\b)' +
  '|(?<keyword>\\b(?:Module|End|Sub|Function|Dim|As|Const|If|Then|Else|ElseIf|For|To|Step|Next|While|Do|Loop|Until|Select|Case|Return|New|AndAlso|OrElse|Not|Mod|True|False)\\b)' +
  '|(?<func>\\b(?:Console|WriteLine|ReadLine|Math|CInt|CStr|CDbl|CBool|Len|Mid|Left|Right|UCase|LCase|Asc|Chr|Rnd|Sqrt|Abs|Round)\\b)',
  'g'
);
...
const PY_TOKEN_RE = new RegExp(
  ...
  '|(?<string>"(?:[^"\\\\]|\\\\.)*"|\'(?:[^\'\\\\]|\\\\.)*\')' +
  '|(?<number>\\b\\d+(?:\\.\\d+)?\\b)' +
  '|(?<keyword>\\b(?:def|if|elif|else|for|while|in|range|return|True|False|None|and|or|not|import|class|break|continue|pass|from|as|global|try|except|finally|raise|with|lambda|yield)\\b)' +
  '|(?<func>\\b(?:print|input|len|str|int|float|abs|round|ord|chr|sqrt|math)\\b)',
  'g'
);

function highlightLine(line, lang) {
  const re = lang === 'vb' ? VB_TOKEN_RE : lang === 'python' ? PY_TOKEN_RE : null;
  ...
  while ((m = re.exec(line)) !== null) {
    if (m.index > lastIndex) out += escHtml(line.slice(lastIndex, m.index));
    const g = m.groups;
    const cls = g.comment ? 'comment' : g.string ? 'str' : g.number ? 'num' :
                g.type ? 'typ' : g.keyword ? 'kw' : g.func ? 'fn' : '';
...
function addCodeOutput(text, type, lang) {
  const body = document.getElementById('outputBody');
  const el = document.createElement('div');
  el.className = 'output-line ' + type;
  el.innerHTML = text.length ? highlightLine(text, lang) : '&nbsp;';
  body.appendChild(el);
...
function parseParams(paramStr) {
  ...
  return paramStr.split(',').map(p => {
    let part = p.trim();
    let passBy = 'BYVAL';
    const byMatch = part.match(/^(BYREF|BYVAL)\s+(.+)/i);
    if (byMatch) { passBy = byMatch[1].toUpperCase(); part = byMatch[2].trim(); }
    const parts = part.split(':');
    return { name: parts[0]?.trim(), type: parts[1]?.trim(), passBy };
  }).filter(p => p.name);
}
...
async function execLines(lines, env, startIdx=0) {
  ...
  while (i < lines.length) {
    const line = lines[i];
    // Skip over FUNCTION/PROCEDURE definition blocks entirely — they were
    // already collected in the first pass, so executing them again here
    // (and hitting their RETURN) would wrongly terminate the whole program.
...
async function callProc(name, argsStr, env) {
  ...
  const args = argTokens.map(a => evalExpr(a, env));
  ...
  // BYREF: write the parameter's final value back into the caller's variable
  // (the variable actually passed in, not the parameter's own name).
  if (proc.params) proc.params.forEach((p, i) => {
    if (p.passBy === 'BYREF' && i < argTokens.length) {
      const argName = argTokens[i].trim();
      if (/^[A-Za-z_]\w*$/.test(argName)) {
        env.vars[argName.toUpperCase()] = localEnv.vars[p.name.toUpperCase()];
      }
    }
  });
}
...
function parseArgList(argsStr, env) {
  return splitArgs(argsStr).map(a => evalExpr(a, env));
}
...
function evalExpr(expr, env) {
  ...
  if (fnCall) {
    ...
    if (env.funcs && env.funcs[fname]) {
      return callFuncSync(fnCall[1], argStr, env);
    }
    const args = splitArgs(argStr).map(a => evalExpr(a, env));
...
function callFuncSync(name, argsStr, env) {
  // Synchronous function call for use inside expressions (e.g. OUTPUT square(5)).
  const upName = name.toUpperCase();
  const fn = env.funcs[upName];
  ...
  const localEnv = { vars: { ...env.vars }, funcs: env.funcs, procs: env.procs, arrays: { ...env.arrays }, callStack: [...(env.callStack||[]), name] };
  if (fn.params) fn.params.forEach((p, i) => { localEnv.vars[p.name.toUpperCase()] = args[i] ?? undefined; });
  const res = execLinesSync(fn.body, localEnv);
  return (res && res.type === 'return') ? res.value : undefined;
}

function callProcSync(name, argsStr, env) {
  const upName = name.toUpperCase();
  const proc = env.procs[upName];
  ...
  const localEnv = { vars: { ...env.vars }, funcs: env.funcs, procs: env.procs, arrays: { ...env.arrays }, callStack: [...(env.callStack||[]), name] };
  if (proc.params) proc.params.forEach((p, i) => { localEnv.vars[p.name.toUpperCase()] = args[i] ?? undefined; });
  execLinesSync(proc.body, localEnv);
  if (proc.params) proc.params.forEach((p, i) => {
    if (p.passBy === 'BYREF' && i < argTokens.length) {
      const argName = argTokens[i].trim();
      if (/^[A-Za-z_]\w*$/.test(argName)) env.vars[argName.toUpperCase()] = localEnv.vars[p.name.toUpperCase()];
    }
  });
  return undefined;
}

// Synchronous statement-block executor: mirrors execLines/execLine but with no
// awaiting, so it can be called from within evalExpr (e.g. OUTPUT f(5)). Used
// for FUNCTION/PROCEDURE bodies, which generally don't need INPUT/file I/O.
function execLinesSync(lines, env, startIdx = 0) {
  let i = startIdx;
  while (i < lines.length) {
    const line = lines[i];
    if (!line || line.startsWith('//') || line.startsWith('REM')) { i++; continue; }

    if (/^(FUNCTION|PROCEDURE)\b/i.test(line)) {
      let depth = 1; i++;
      while (i < lines.length && depth > 0) {
        if (/^(FUNCTION|PROCEDURE)\b/i.test(lines[i])) depth++;
        if (/^(ENDFUNCTION|ENDPROCEDURE)\b/i.test(lines[i])) depth--;
        i++;
      }
      continue;
    }
    ...
    if (/^DECLARE\s/i.test(line)) {
      ...
      if (m) {
        ...
        if (arrMatch) {
          const lo = parseInt(arrMatch[1]), hi = parseInt(arrMatch[2]);
          env.arrays[name.toUpperCase()] = { lo, hi, data: new Array(hi - lo + 1).fill(undefined) };
        } else if (!(name.toUpperCase() in env.vars)) {
          env.vars[name.toUpperCase()] = defaultVal(typeInfo);
        }
      }
      i++; continue;
    }
    ...
    if (/^RETURN\s/i.test(line)) {
      return { type: 'return', value: evalExpr(line.replace(/^RETURN\s+/i, ''), env) };
    }

    if (/^CALL\s+/i.test(line)) {
      const m = line.match(/^CALL\s+(\w+)\s*(?:\(([^)]*)\))?/i);
      if (m) callProcSync(m[1], m[2] || '', env);
      i++; continue;
    }
    ...
    if (assignArr) {
      ...
      const idx2 = Math.round(evalExpr(assignArr[2], env));
      const val = evalExpr(assignArr[3], env);
      if (env.arrays[name]) env.arrays[name].data[idx2 - env.arrays[name].lo] = val;
      i++; continue;
    }
    if (assignSimple) {
      env.vars[assignSimple[1].toUpperCase()] = evalExpr(assignSimple[2], env);
      i++; continue;
    }

    if (/^(OUTPUT|PRINT)\s/i.test(line)) {
      addOutput(String(evalExpr(line.replace(/^(OUTPUT|PRINT)\s+/i, ''), env)));
      i++; continue;
    }
    ...
    if (/^IF\s.+THEN$/i.test(line)) {
      ...
      const res = evalCond(cond, env) ? execLinesSync(ifLines, env) : execLinesSync(elseLines, env);
      if (res && res.type === 'return') return res;
      i = j + 1; continue;
    }
    ...
    if (forMatch) {
      ...
      for (let v = start; step > 0 ? v <= end : v >= end; v += step) {
        if (++guard > 100000) throw new Error('Loop limit exceeded');
        env.vars[varName] = v;
        const res = execLinesSync(body, env);
        if (res && res.type === 'return') return res;
      }
      i = j + 1; continue;
    }
    ...
    // Bare function/procedure call without CALL keyword
    const callNoKeyword = line.match(/^(\w+)\s*\(([^)]*)\)\s*$/);
    if (callNoKeyword) {
      const name = callNoKeyword[1].toUpperCase();
      if (env.procs[name]) { callProcSync(callNoKeyword[1], callNoKeyword[2], env); i++; continue; }
      if (env.funcs[name]) { callFuncSync(callNoKeyword[1], callNoKeyword[2], env); i++; continue; }
    }
...
// ─── Built-in pseudocode functions (LENGTH, SUBSTRING, ROUND, RANDOM, etc.) ──
function callBuiltin(fname, args, argStr, env) {
  switch (fname) {
    ...
    case 'SUBSTRING': { // SUBSTRING(string, start, length) — 1-indexed
      const s = String(args[0]); const start = Math.round(args[1]); const len = Math.round(args[2]);
      return s.substr(start - 1, len);
    }
    case 'MID': { const s = String(args[0]); const start = Math.round(args[1]); const len = args.length > 2 ? Math.round(args[2]) : s.length; return s.substr(start - 1, len); }
    case 'LEFT': return String(args[0]).substring(0, Math.round(args[1]));
    case 'RIGHT': { const s = String(args[0]); const n = Math.round(args[1]); return s.substring(s.length - n); }
    case 'ROUND': return args.length > 1 ? Number(Number(args[0]).toFixed(Math.round(args[1]))) : Math.round(args[0]);
    case 'INT': return Math.trunc(args[0]);
    case 'ABS': return Math.abs(args[0]);
    case 'RANDOM': case 'RAND': return Math.random();
    case 'RANDOMBETWEEN': { const lo = Math.round(args[0]), hi = Math.round(args[1]); return lo + Math.floor(Math.random() * (hi - lo + 1)); }
    case 'ASC': return String(args[0]).charCodeAt(0);
    case 'CHR': return String.fromCharCode(Math.round(args[0]));
    case 'STR_TO_NUM': case 'NUM': return parseFloat(args[0]);
    case 'NUM_TO_STR': case 'STR': return String(args[0]);
    case 'EOF': {
      const filename = String(args[0]);
      const handle = vfsHandles[filename];
      const f = vfsFiles[filename];
      if (!handle || !f) return true;
      return handle.pos >= f.lines.length;
    }
...
function convertVBLines(lines, out, indentLevel) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^(ENDFUNCTION|ENDPROCEDURE)\b/i.test(line)) continue; // stripped upstream, safety net only
...
<!-- Floating built-in functions button -->
<div class="builtin-fab" id="builtinFab">
  <div class="builtin-popover" id="builtinPopover">
    <div class="builtin-pop-header" id="builtinPopHeader">🔧 Built-in Functions</div>
    <div class="builtin-pop-body" id="builtinList"></div>
  </div>
  <button class="builtin-fab-btn" id="builtinFabBtn" onclick="toggleBuiltinPopover()">
    🔧 <span id="builtinFabLabel">Built-ins</span>
  </button>
</div>
